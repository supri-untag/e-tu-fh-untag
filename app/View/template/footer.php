<script src="<?= $model['url']?>/assets/js/jquery-3.3.1.min.js"></script>
<script src="<?= $model['url']?>/assets/js/popper.min.js" ></script>
<script src="<?= $model['url']?>/assets/js/bootstrap.min.js" ></script>
<script src="<?= $model['url']?>/assets/js/jquery.nicescroll.min.js"></script>
<script src="<?= $model['url']?>/assets/js/moment.min.js"></script>
<script src="<?= $model['url']?>/assets/js/stisla.js"></script>

<!-- JS Libraies -->
<script src="<?= $model['url']?>/node_modules/jquery-sparkline/jquery.sparkline.min.js"></script>
<script src="<?= $model['url']?>/node_modules/chart.js/dist/Chart.min.js"></script>
<script src="<?= $model['url']?>/node_modules/owl.carousel/dist/owl.carousel.min.js"></script>
<script src="<?= $model['url']?>/node_modules/summernote/dist/summernote-bs4.js"></script>
<script src="<?= $model['url']?>/node_modules/chocolat/dist/js/jquery.chocolat.min.js"></script>
<script src="<?= $model['url']?>/node_modules/select2/dist/js/select2.full.min.js"></script>
<script src="<?= $model['url']?>/assets/modules/datatables.min.js"></script>
<script src="<?= $model['url']?>/node_modules/dropzone/dist/min/dropzone.min.js"></script>

<!-- JS Module -->
<script src="<?= $model['url']?>/node_modules/pwstrength/dist/pwstrength-bootstrap.min.js"></script>
<script src="<?= $model['url']?>/assets/js/sw2/dist/sweetalert2.all.min.js"></script>

<!-- Template JS File -->
<script src="<?= $model['url']?>/assets/js/scripts.js"></script>
<script src="<?= $model['url']?>/assets/js/app.js"></script>


</body>
</html>